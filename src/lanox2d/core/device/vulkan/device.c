...
#   define LX_DEVICE_DESCRIPTOR_SETS_GROW     (8)
#else
#   define LX_DEVICE_VERTEX_BUFFERS_GROW      (128)
#   define LX_DEVICE_DESCRIPTOR_SETS_GROW     (16)
#endif
...
static lx_void_t lx_device_vulkan_exit(lx_device_ref_t self) {
    ...
    if (device) {
        ...
        if (device->renderer_descriptor_sets) {
            lx_array_exit(device->renderer_descriptor_sets);
            device->renderer_descriptor_sets = lx_null;
        }
...
lx_device_ref_t lx_device_init_from_vulkan(lx_size_t width, lx_size_t height, lx_cpointer_t vkinstance, lx_cpointer_t vksurface) {
    ...
    do {
        ...
        device->renderer_descriptor_sets = lx_array_init(LX_DEVICE_DESCRIPTOR_SETS_GROW, lx_element_mem(sizeof(VkDescriptorSet), lx_null, lx_null));
        lx_assert_and_check_break(device->renderer_descriptor_sets);
