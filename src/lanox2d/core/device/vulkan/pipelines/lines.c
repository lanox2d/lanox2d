 ...
 */
static lx_bool_t lx_vk_descriptor_sets_init_lines(lx_vulkan_device_t* device, lx_vk_pipeline_t* pipeline, const lx_uint32_t descriptor_type, const lx_uint32_t descriptor_count) {
...
lx_vk_pipeline_ref_t lx_vk_pipeline_lines(lx_vulkan_device_t* device) {
    const lx_size_t type = LX_VK_PIPELINE_TYPE_LINES;
...
#include "solid.frag.spv.h"
        ...
        lx_vk_pipeline_t* pipeline_lines = lx_null;
        do {
            pipeline_lines = lx_vk_pipeline_init(device, type);
            lx_assert_and_check_break(pipeline_lines);
            ...
            if (vkCreateDescriptorSetLayout(device->device, &descriptor_set_layout_info, lx_null, &pipeline_lines->descriptor_set_layout) != VK_SUCCESS) {
                break;
            }
            ...
            pipeline_layout_info.pSetLayouts = &pipeline_lines->descriptor_set_layout;
            pipeline_layout_info.pushConstantRangeCount = 1;
            pipeline_layout_info.pPushConstantRanges = &push_constant_range;

            // init descriptor sets
            if (!lx_vk_descriptor_sets_init_lines(device, pipeline_lines, descriptor_type, descriptor_count)) {
                break;
            }

            // create pipeline
            if (!lx_vk_pipeline_create(pipeline_lines, VK_PRIMITIVE_TOPOLOGY_LINE_STRIP,
                vshader, sizeof(vshader), fshader, sizeof(fshader), &vertex_input_info, &pipeline_layout_info)) {
                break;
            }

            // ok
            pipeline = (lx_vk_pipeline_ref_t)pipeline_lines;
            device->pipelines[type] = pipeline;
            ok = lx_true;
        } while (0);

        if (!ok && pipeline_lines) {
            lx_vk_pipeline_exit((lx_vk_pipeline_ref_t)pipeline_lines);
            pipeline_lines = lx_null;
        }
    }
    return pipeline;
}
