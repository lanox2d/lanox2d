 ...
 */
static lx_size_t lx_bitmap_renderer_stroke_line_generic(lx_bitmap_biltter_t* biltter, lx_fixed6_t xb, lx_fixed6_t yb, lx_fixed6_t xe, lx_fixed6_t ye) {
    // round coordinates
...
static lx_void_t lx_bitmap_renderer_stroke_line_vertical(lx_bitmap_biltter_t* biltter, lx_fixed6_t xb, lx_fixed6_t yb, lx_fixed6_t xe, lx_fixed6_t ye) {
    // ensure the order
    if (yb > ye) {
...
lx_void_t lx_bitmap_renderer_stroke_lines(lx_bitmap_device_t* device, lx_point_ref_t points, lx_size_t count) {
    ...
    for (i = 0; i < count; i += 2) {
        ...
        // draw line
        if ((ok = lx_bitmap_renderer_stroke_line_generic(&device->biltter, xb, yb, xe, ye))) {
            lx_assert(ok == 'h' || ok == 'v');
            if (ok == 'h') {
                lx_bitmap_renderer_stroke_line_horizontal(&device->biltter, xb, yb, xe, ye);
            } else {
                lx_bitmap_renderer_stroke_line_vertical(&device->biltter, xb, yb, xe, ye);
            }
        }
    }
}
