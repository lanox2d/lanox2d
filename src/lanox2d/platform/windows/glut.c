...
static lx_bool_t lx_window_glut_start(lx_window_glut_t* window) {
    ...
    do {
        ...
        window->base.device = lx_device_init_from_opengl(window->base.width, window->base.height, window->base.width, window->base.height);
 ...
 * implementation
 */
lx_window_ref_t lx_window_init_glut(lx_size_t width, lx_size_t height, lx_char_t const* title, lx_cpointer_t devdata) {
    lx_bool_t         ok = lx_false;
