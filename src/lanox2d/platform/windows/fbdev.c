...
static lx_bool_t lx_window_fbdev_start(lx_window_fbdev_t* window) {
    ...
    do {
        ...
        // activate buffer
        window->vinfo.activate = FB_ACTIVATE_NOW | FB_ACTIVATE_FORCE;
 ...
 * implementation
 */
lx_window_ref_t lx_window_init_fbdev(lx_size_t width, lx_size_t height, lx_char_t const* title, lx_cpointer_t devdata) {
    lx_bool_t          ok = lx_false;
