...
lx_size_t lx_rforeach(lx_iterator_ref_t iterator, lx_size_t head, lx_size_t tail, lx_rforeach_cb_t callback, lx_cpointer_t udata) {
    ...
    lx_size_t count = 0;
    lx_rfor (lx_pointer_t, item, head, tail, iterator) {
        if (!callback(iterator, item, udata)) break;
        count++;
    }
    return count;
}
lx_size_t lx_rforeach_all(lx_iterator_ref_t iterator, lx_rforeach_cb_t callback, lx_cpointer_t udata) {
    lx_assert(iterator && callback);
    return lx_rforeach(iterator, lx_iterator_head(iterator), lx_iterator_tail(iterator), callback, udata);
}
